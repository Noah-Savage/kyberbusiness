  ...
  const handlePaymentSuccess = async (details) => {
    try {
      const response = await fetch(
        `${API_URL}/public/invoices/${id}/mark-paid?payment_id=${details.id}`,
        { method: "POST" }
      );
      if (response.ok) {
        setPaymentSuccess(true);
      ...
      toast.error("Payment recorded but failed to update invoice. Please contact support.");
    }
  };
          ...
          <Card className="lg:col-span-2 rounded-3xl bg-card/50 backdrop-blur-xl border-white/10">
            <CardContent className="p-6 md:p-8">
              <div className="flex items-start justify-between mb-6">
                ...
                <Badge className="bg-cyan-500/20 text-cyan-500 capitalize text-sm px-4 py-1">
                  {invoice.status}
                </Badge>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                {invoice.items.map((item, index) => (
                  <div key={index} className="grid grid-cols-12 text-sm py-2">
                ...
                {invoice.paypal_client_id ? (
                  ...
                  <div className="text-center p-4 rounded-xl bg-muted/50">
                    ...
                    <p className="text-sm text-muted-foreground">
                      PayPal is not configured. Please contact the business to arrange payment.
                    </p>
                  </div>
                )}
              </CardContent>
